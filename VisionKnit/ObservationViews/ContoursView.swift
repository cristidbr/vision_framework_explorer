...
struct ContoursView: View
{
    ...
    var body: some View
    {
        Canvas {
            ...
            context.stroke( Path( top_path.copy( using: &transform ) ?? top_path ), with: .color( Color.accentColor ), lineWidth: 2 );
            
            context.stroke( Path( sub_path.copy( using: &transform ) ?? sub_path ), with: .color( Color.accentColor ), lineWidth: 1 );
        }
