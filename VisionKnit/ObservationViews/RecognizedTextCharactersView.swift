...
struct RecognizedTextCharactersView: View
{
    ...
    public init( _ text: String = "", uuid: UUID, cgImage: CGImage, boundingBox: CGRect = CGRect.null, characterBoxes: [ CGRect ] = [], rotation: Double = 0 )
    {
        self.text = text;
    ...
    {
        let rowlimit = 9;
        let glayout: [ GridItem ] = Array( repeating: .init( .adaptive( minimum: boundingBox.height ) ), count: min( rowlimit, self.characterBoxes.count ) );
...
struct RecognizedTextCharactersView_Previews: PreviewProvider
{
    ...
    {
        RecognizedTextCharactersView( "OCR Text", uuid: UUID(), cgImage: loadCGImage( VisionAPIDescription[ 0 ].samples[ 0 ].path ), boundingBox: .init( x: 100, y: 75, width: 50, height: 25 ) )
    }
}
