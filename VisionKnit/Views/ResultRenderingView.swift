...
struct ResultRenderingView: View {
    ...
    init(_ observations: [VNObservation]?, completed: Bool) {
    ...
    @ViewBuilder private func renderFailMessage(_ message: String) -> some View {
        VStack(alignment: .center) {
            Spacer()
            HStack(alignment: .center) {
                Spacer()
    ...
    @ViewBuilder private func displayClassifications(_ array: [VNClassificationObservation])
        -> some View
    {
    ...
    @ViewBuilder private func displayBarcodes(_ array: [VNBarcodeObservation], areaSize: CGSize)
        -> some View
    {
    ...
    @ViewBuilder private func displayText(_ array: [VNRecognizedTextObservation], areaSize: CGSize)
        -> some View
    {
        ZStack {
            ForEach(array, id: \.uuid) {
                ...
                let bboxSize = observationMeasureCornerPointsBoxRotatedSize(
                    topLeft: observationPointToImagePoint(observation.topLeft, size: areaSize),
                ...
                RecognizedTextView(
                    label,
                    boundingBox: CGRectInset(
                        bbox, (bbox.width - bboxSize.width) / 2, (bbox.height - bboxSize.height) / 2),
    ...
    @ViewBuilder private func displayDocumentSegmentation(_ array: [VNRectangleObservation])
        -> some View
    {
        ZStack {
    ...
    @ViewBuilder private func displayDocumentPoly(_ array: [VNRectangleObservation]) -> some View {
        ForEach(array, id: \.uuid) {
            ...
            RecognizedPolygonView(
                topLeft: observationPointInvertY(observation.topLeft),
                bottomLeft: observationPointInvertY(observation.bottomLeft),
                bottomRight: observationPointInvertY(observation.bottomRight),
                topRight: observationPointInvertY(observation.topRight))
        }
    }
    ...
    @ViewBuilder private func displayFaceObservations(
        _ array: [VNFaceObservation], areaSize: CGSize, landmarks: Bool
    ) -> some View {
        if landmarks {
    ...
    @ViewBuilder private func displayFaceRectangles(_ array: [VNFaceObservation], areaSize: CGSize)
        -> some View
    {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displayFaceLandmarks(_ array: [VNFaceObservation], areaSize: CGSize)
        -> some View
    {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displayTextRectangles(_ array: [VNTextObservation], cgImage: CGImage)
        -> some View
    {
            ...
            VStack(spacing: 0) {
                VStack(spacing: 8) {
                    ForEach(array, id: \.uuid) {
                        ...
                        let cbboxes = cboxes.map {
                            observationBoundingBoxToArea($0.boundingBox, areaSize: imageSize)
                        }

                        RecognizedTextCharactersView(
                            "", uuid: observation.uuid, cgImage: cgImage, boundingBox: bbox,
                            characterBoxes: cbboxes)
                    }
                }
                .padding(.all, 8)
    ...
    @ViewBuilder private func displayHorizon(_ array: [VNHorizonObservation], cgImage: CGImage)
        -> some View
    {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displayHumanRectangles(_ array: [VNHumanObservation], areaSize: CGSize)
        -> some View
    {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displayContours(_ array: [VNContoursObservation], areaSize: CGSize)
        -> some View
    {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displayHumanBodyPose(
        _ array: [VNHumanBodyPoseObservation], areaSize: CGSize
    ) -> some View {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displayHumanHandPose(
        _ array: [VNHumanHandPoseObservation], areaSize: CGSize
    ) -> some View {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displayLabeledObjects(
        _ array: [VNRecognizedObjectObservation], areaSize: CGSize
    ) -> some View {
        ForEach(array, id: \.uuid) {
            observation in
    ...
    @ViewBuilder private func displaySaliency(
        _ array: [VNSaliencyImageObservation], areaSize: CGSize, boxes: Bool
    ) -> some View {
        ZStack {
            ForEach(array, id: \.uuid) {
                observation in
    ...
    @ViewBuilder private func displayFeaturePrint(_ array: [VNFeaturePrintObservation]) -> some View {
        ForEach(array, id: \.uuid) {
            observation in
            FeaturePrintView(observation.toDrawableCGPath())
        }
    }
...
struct ResultRenderingView_Previews: PreviewProvider {
    static var previews: some View {
        ResultRenderingView([], completed: false)
